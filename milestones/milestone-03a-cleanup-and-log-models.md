...
- [x] Create `Models/LogListResponse.cs` — `public record LogListResponse(string ProjectId, string RunId, IReadOnlyList<LogItemInfo> Logs, IReadOnlyList<LogItemInfo> Prompts, IReadOnlyList<LogItemInfo> Artifacts)` in namespace `LogViewerApi.Models`
